...
namespace Routrek.SSHC {
	...
	internal abstract class SSHDataReader {
		...
		public int ReadInt32() {
			if(_offset+3>=_data.Length) throw new IOException(ErrorInfo.GetErrorMessageText(ErrorSshMessage.UnexpectedEOF));
		...
		public byte ReadByte() {
			if (_offset >= _data.Length)
				throw new IOException(ErrorInfo.GetErrorMessageText(ErrorSshMessage.UnexpectedEOF));
