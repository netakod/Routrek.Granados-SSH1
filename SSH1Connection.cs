...
namespace Routrek.SSHCV1
{
	public sealed class SSH1Connection : SSHConnection {
		...
		internal override AuthenticationResult Connect(AbstractSocket s) {
			...
			if(ReceiveAuthenticationRequirement()==AUTH_REQUIRED) {
				...
				if (!auth)
					throw new SSHException(ErrorInfo.GetErrorMessageText(ErrorSshMessage.AuthenticationFailed));
		...
		internal async ValueTask TransmitAsync(SSH1Packet p)
		{
			await p.WriteToAsync(_stream, _tCipher);
		}
		...
		private void ReceiveServerKeys() {
			...
			*/
			...
			if(!found)
				throw new SSHException(String.Format(ErrorInfo.GetErrorMessageText(ErrorSshMessage.ServerNotSupportedX), "Blowfish/TripleDES"));
			...
			if(_param.AuthenticationType==AuthenticationType.Password && (supported_authentications_mask & (1 << (int)AuthenticationType.Password))==0)
				throw new SSHException(String.Format(ErrorInfo.GetErrorMessageText(ErrorSshMessage.ServerNotSupportedPassword), SSH1Packet.Data));
			if(_param.AuthenticationType==AuthenticationType.PublicKey && (supported_authentications_mask & (1 << (int)AuthenticationType.PublicKey))==0)
				throw new SSHException(String.Format(ErrorInfo.GetErrorMessageText(ErrorSshMessage.ServerRefusedRSA)), SSH1Packet.Data);
		}
		...
		private void DoRSAChallengeResponse() {
			...
			if(p.Type==PacketType.SSH_SMSG_FAILURE)
				throw new SSHException(ErrorInfo.GetErrorMessageText(ErrorSshMessage.ServerRefusedRSA));
			else if(p.Type!=PacketType.SSH_SMSG_AUTH_RSA_CHALLENGE)
				throw new SSHException(String.Format(ErrorInfo.GetErrorMessageText(ErrorSshMessage.UnexpectedResponse), p.Type));
	...
	public class SSH1Channel : SSHChannel {
		...
		*/
		...
		public override async ValueTask TransmitAsync(byte[] data)
		{
			SSH1DataWriter wr = new SSH1DataWriter();
			if (_type == ChannelType.Shell)
			{
		...
		*/
		...
		public override async ValueTask TransmitAsync(byte[] data, int offset, int length)
		{
			SSH1DataWriter wr = new SSH1DataWriter();
		...
		private async ValueTask TransmitAsync(SSH1Packet p)
		{
			await ((SSH1Connection)_connection).TransmitAsync(p);
		}
	}
