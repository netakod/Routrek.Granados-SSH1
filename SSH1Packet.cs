...
namespace Routrek.SSHCV1
{
	...
	internal class SSH1Packet
	{
		...
		*/
		...
		public async ValueTask WriteToAsync(AbstractSocket output)
		{
			byte[] image = BuildImage();
			
			await output.WriteAsync(image, 0, image.Length);
		}
		...
		*/
		...
		{
			byte[] image = EncryptImage(output, cipher);
			
			output.Write(image, 0, image.Length);
		}

		public async ValueTask WriteToAsync(AbstractSocket output, Cipher cipher)
		{
			byte[] image = EncryptImage(output, cipher);
		...
		private byte[] EncryptImage(AbstractSocket output, Cipher cipher)
		{
			byte[] image = BuildImage();
			//dumpBA(image);
