	...
	public class SSH1UserAuthKey
	{
		...
		public SSH1UserAuthKey(string path, string passphrase)
		{
			...
			if(Encoding.ASCII.GetString(header)!="SSH PRIVATE KEY FILE FORMAT 1.1\n")
				throw new SSHException(String.Format(ErrorInfo.GetErrorMessageText(ErrorSshMessage.BrokenKeyFile), path));
			...
			if(mark[0]!=mark[2] || mark[1]!=mark[3])
				throw new SSHException(ErrorInfo.GetErrorMessageText(ErrorSshMessage.WrongPassphrase));
