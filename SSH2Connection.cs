...
namespace Routrek.SSHCV2
{
	public sealed class SSH2Connection : SSHConnection {
		...
		internal async ValueTask<SSH2Packet> TransmitPacketAsync(byte[] payload)
		{
			SSH2Packet p = SSH2Packet.FromPlainPayload(payload, _tCipher == null ? 8 : _tCipher.BlockSize, _param.Random);
			if (_tMAC != null) p.CalcHash(_tMAC, _tSequence);
	...
	public class SSH2Channel : SSHChannel {
		...
		public override void Transmit(byte[] data)	
		{
			...
			byte[] packageData = this.CreatePackage(data, 0, data.Length);

			this.TransmitPacket(packageData);
		}
		...
		public override async ValueTask TransmitAsync(byte[] data)
		{
			...
			await this.TransmitPacketAsync(packageData);
		}

		public override void Transmit(byte[] data, int offset, int length)	
		{
			byte[] packageData = this.CreatePackage(data, offset, length);
		...
		private byte[] CreatePackage(byte[] data, int offset, int length)
		{
			SSH2DataWriter wr = new SSH2DataWriter();
		...
		public override async ValueTask CloseAsync()
		{
			...
			await this.TransmitPacketAsync(wr.ToByteArray());
		}
		...
		private async ValueTask TransmitPacketAsync(byte[] data)
		{
			await ((SSH2Connection)_connection).TransmitPacketAsync(data);
		}
