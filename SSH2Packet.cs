...
namespace Routrek.SSHCV2
{
	...
	internal class SSH2Packet
	{
		...
		{
			var buf = this.CreateDataForWrite(strm, cipher);

			strm.Write(buf, 0, buf.Length);
			strm.Flush();
		}

		public async ValueTask WriteToAsync(AbstractSocket strm, Cipher cipher)
		{
			var buf = this.CreateDataForWrite(strm, cipher);

			await strm.WriteAsync(buf, 0, buf.Length);
			strm.Flush();
		}

		private byte[] CreateDataForWrite(AbstractSocket strm, Cipher cipher)
		{
