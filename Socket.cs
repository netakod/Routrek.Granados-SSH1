...
namespace Routrek.SSHC
{
	...
	internal class ProtocolNegotiationHandler : SynchronizedHandlerBase, IByteArrayHandler {
		...
		public ProtocolNegotiationHandler(SSHConnectionParameter param) {
			_param = param;
			_errorMessage = ErrorInfo.GetErrorMessageText(ErrorSshMessage.NotSSHServer);
		}
		...
		public void OnData(byte[] buf, int offset, int length) {
			try {
				...
				if (length <= 2 || buf[offset + length - 1] != 0x0A)
					throw new SSHException(ErrorInfo.GetErrorMessageText(ErrorSshMessage.NotSSHServer));
	...
	internal abstract class AbstractSocket {
		...
		internal abstract ValueTask WriteAsync(byte[] data, int offset, int length);

		internal abstract void WriteByte(byte data);
		internal abstract ValueTask WriteByteAsync(byte data);
	...
	internal class PlainSocket : AbstractSocket {
		...
		internal override async ValueTask WriteAsync(byte[] data, int offset, int length)
		{
			await _socket.SendAsync(new ArraySegment<byte>(data, offset, length), SocketFlags.None);
		}

		internal override void WriteByte(byte data) 
		{
			_socket.Send(new byte[] { data }, 0, 1, SocketFlags.None);
		}

		internal override async ValueTask WriteByteAsync(byte data)
		{
			await _socket.SendAsync(new ArraySegment<byte>(new byte[] { data }, 0, 1), SocketFlags.None);
		}
	...
	internal class ChannelSocket : AbstractSocket, ISSHChannelEventReceiver {
		...
		internal override void WriteByte(byte data) 
		{
			...
			_channel.Transmit(new byte[] { data });
		}

		internal async override ValueTask WriteByteAsync(byte data)
		{
			if (!_ready || _channel == null)
				throw new SSHException("channel not ready");

			await _channel.TransmitAsync(new byte[] { data });
		}
		...
		internal override async ValueTask WriteAsync(byte[] data, int offset, int length)
		{
			...
			await _channel.TransmitAsync(data, offset, length);
		}
